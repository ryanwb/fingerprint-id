 ...
 * Build me: gcc -Wall -Wextra qdbmp.c nonlcdk_run_bmpout_adaptive.c -o nonlcdk_run_bmpout_adaptive.out
 */
...
int main(void)
{
    ...
    for (k = 0; k < N_FPRINTS * N_EACH; k++)
    {
        ...
        grey = to_greyscale(bitmap[k], width[k], height[k]); // load the image in black & white
        ...
        */
        ...
        upsidedown(skeleton, width[k], height[k]);
        // Save result
        BMP* out_bmp = bmp_from_binary_array(skeleton, width[k], height[k], depth[k]);
        m_free(skeleton);
        char str[80];
        sprintf(str, "skele_%s", files[k]);
        BMP_WriteFile(out_bmp, str);
    ...
    while(1)
    {
        ...
        printf("Converting bitmap to greyscale...\n");
        grey = to_greyscale(bitmap[fid], width[fid], height[fid]); // load the image in black & white
